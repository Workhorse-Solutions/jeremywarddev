...
## `rails_foundry_cli` Gem Workflow

The `rails_foundry_cli` gem lives in the separate [`rails-foundry-cli`](https://github.com/Workhorse-Solutions/rails-foundry-cli) repository. This section documents how to develop, test, and release gem changes from this template repo.

### Local development (iterating on gem changes)

The `rails_foundry_cli` gem is developed as a subdirectory of this repo. It is gitignored (`/rails_foundry_cli/` in `.gitignore`) because it is its own git repository. If the directory is missing (e.g. after a fresh clone or an accidental deletion), restore it with:

```bash
git clone git@github.com:Workhorse-Solutions/rails-foundry-cli.git rails_foundry_cli
```

Point the Gemfile at the local subdirectory instead of the git tag while iterating:

```ruby
# Gemfile — temporary, do not commit
gem "rails_foundry_cli", path: "rails_foundry_cli"
```

Then re-bundle and reinstall tooling:
...
Make and test your gem changes. When done, revert the Gemfile to the tag reference before committing.

### Releasing a new gem version

1. In the `rails-foundry-cli` repo: make changes, bump the version, tag the release (e.g. `v0.1.1`), and push the tag.
2. In this repo: update the `tag:` reference in the Gemfile (use HTTPS, not SSH, so subscriber CI can authenticate with a token):
   ```ruby
   gem "rails_foundry_cli", git: "https://github.com/Workhorse-Solutions/rails-foundry-cli.git", tag: "v0.1.1"
   ```
3. Run `bundle update rails_foundry_cli` to update `Gemfile.lock`.
4. If skills, hooks, or settings changed in the gem, re-run the generator:
   ```bash
   rails generate rails_foundry_cli:ai_tooling
   ```
   The generator will prompt before overwriting any locally modified files.
5. Commit `Gemfile`, `Gemfile.lock`, and any regenerated `.claude/` files together.

### Subscriber CI setup

Because `rails-foundry-cli` is a private repo, subscribers must authenticate to clone it during `bundle install`. The template's `ci.yml` reads `BUNDLE_GITHUB__COM` from a secret named `BUNDLE_GITHUB_TOKEN`.

Each subscriber must:

1. Create a GitHub PAT (classic) with `repo` scope, or a fine-grained token with **read-only Contents** access to `rails-foundry-cli`.
2. Add it as a secret named `BUNDLE_GITHUB_TOKEN` in their app repo's **Settings → Secrets and variables → Actions**.

Access is revoked by removing the subscriber from the GitHub org/team that has read access to `rails-foundry-cli`.

### After any gem upgrade

Always re-run the generator after upgrading the gem tag to pull in updates to skills and hooks:
